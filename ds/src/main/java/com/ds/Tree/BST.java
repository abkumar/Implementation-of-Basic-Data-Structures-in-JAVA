 ...
 *
 * @Buffalo Cracking the Coding Interview
...
//		5) If current is NULL and stack is empty then we are done.
  ...
  public static boolean isBST(Node node) {
    ...
    if (node != null) {
      ...
      if (node.getLeft() == null && node.getRight() != null) {
        result = node.getRight().compareTo(node.getData()) >= 0 && isBST(node.getRight());
      } else {
  ...
  private boolean hasPathSum(Node node, int sum) {
    ...
    if (node != null) {
      ...
      result = hasPathSum(node.getLeft(), subSum) || hasPathSum(node.getRight(), subSum);
    }
    return result;
  }
